...
void solve() {
  ...
  rep (i, d) {
    ...
    last_open[t[i]] = i;
    rep (j, n) {
      ans -= c[j] * (i - last_open[j]);
    }
  }
  // print(ans);

  vector<set<int>> opendays(n);
  rep (i, d) {
    opendays[t[i]].insert(i);
  }
  rep (i, n) {
    opendays[i].insert(-1);
    opendays[i].insert(d);
  ...
  rep (i, m) {
    ...
    ans -= s[di][t[di]];
    ans += s[di][qi];
    auto itr = opendays[t[di]].upper_bound(di);
    int left, right;
    right = *itr;
    itr--; itr--;
    left = *itr;
    ans -= c[t[di]] * (di - left) * (right - di);
    opendays[t[di]].erase(di);

    itr = opendays[qi].upper_bound(di);
    right = *itr;
    itr--;
    left = *itr;
    ans += c[qi] * (di - left) * (right - di);
    opendays[qi].insert(di);
    print(ans);
    t[di] = qi;
  }
}
