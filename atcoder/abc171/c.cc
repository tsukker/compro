...
string get_name(int length, ll order) {
  debug2(length, order);
  if (length == 0) {
    return "";
  }
  char head = getchar(order / po26.at(length - 1));
  return head + get_name(length - 1, order % po26.at(length - 1));
}
