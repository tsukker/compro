...
void solve() {
  ...
  FOR (i, 1, n) {
    // [0, i), [1, i), ..., [i, i) (empty)
    ll current = prev;
    FOR (j, 1, (int)mcv.at(i).size()) {
      ll coef = -((int)mcv.at(i).size() - j - 1);
      ll num = mcv.at(i).at(j) - mcv.at(i).at(j - 1);
      current += coef * num;
    }
    current += 1 * ((ll)i - mcv.at(i).back());
    debug3(i, prev, current);
    ans += current;
    prev = current;
  }
  print(ans);
