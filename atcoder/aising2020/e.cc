...
ll aux(vvll& values) {
  multiset<ll> pool;
  rep (i, n+1) {
    for (const ll val : values.at(i)) {
      pool.insert(val);
    }
    while (i < (int)pool.size()) {
      pool.erase(pool.begin());
    }
  }
  ll ret = 0;
  for (const ll val : pool) {
    ret += val;
  }
  return ret;
}
...
void solve() {
  ...
  rep (i, n) {
    ...
    if (l[i] >= r[i]) {
      base_sum += r[i];
      valuesl.at(k[i]).push_back(l[i] - r[i]);
    } else {
      base_sum += l[i];
      valuesr.at(n - k[i]).push_back(r[i] - l[i]);
    }
  }
  ll ans = base_sum + aux(valuesl) + aux(valuesr);
  print(ans);
}
